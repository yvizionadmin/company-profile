...
export function Menu({ open, onClose }: MenuProps) {
  ...
  return (
    ...
    >
      ...
      <div
        data-menu-panel
        className="relative m-3 max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-3xl bg-white p-6 text-ink shadow-2xl sm:p-10 lg:m-5 lg:p-12 will-change-transform"
      >
        {/* header row */}
        <div className="flex items-center justify-between" data-menu-item>
          <button
            type="button"
            onClick={onClose}
            data-cursor="link"
          ...
          <TransitionLink
            ...
            onNavigate={onClose}
            data-cursor="link"
            className="btn bg-ink text-white max-sm:hidden"
          >
        ...
        {/* services */}
        <p className="label-mono mt-10 text-ink/50" data-menu-item>
          Service
        </p>
        ...
        {/* explore */}
        <p className="label-mono mt-12 text-ink/50" data-menu-item>
          Explore
        </p>
        <div className="mt-4 flex flex-wrap gap-x-10 gap-y-3" data-menu-item>
          {EXPLORE.map((item) => (
            <TransitionLink
              key={item.label}
              href={item.href}
              onNavigate={onClose}
              data-cursor="link"
              className="link-sweep text-2xl font-semibold tracking-tight"
            >
              {item.label}
            </TransitionLink>
          ))}
